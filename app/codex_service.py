...
class CodexService:
    ...
    @staticmethod
    def _noninteractive_approval_mode() -> ApprovalMode:
        """Return the SDK's no-prompt approval mode across beta versions.

        Current openai-codex releases call this mode ``deny_all``. It maps to
        app-server's ``approvalPolicy=never``: requests that would require an
        interactive escalation are denied instead of blocking the proxy.
        """
        mode = getattr(ApprovalMode, "deny_all", None)
        if mode is not None:
            return mode

        # Compatibility with any older beta that exposed the protocol name.
        mode = getattr(ApprovalMode, "never", None)
        if mode is not None:
            return mode

        supported = ", ".join(ApprovalMode.__members__)
        raise RuntimeError(
            "No non-interactive ApprovalMode is available; "
            f"installed modes: {supported}"
        )
        ...
        *,
        ...
        async with self.semaphore:
            ...
            kwargs = {
                ...
                "approval_mode": self._noninteractive_approval_mode(),
